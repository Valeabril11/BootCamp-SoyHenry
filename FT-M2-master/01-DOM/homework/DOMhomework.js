...
span.innerHTML = span.innerHTML + ' Valeabril<3';


// var span = document.querySelector('#createdBy');
// span.innerHTML = span.innerHTML + 'Valeabril'
...
function ToDo (description) { // funcion constructora
...
// Agregar un metodo  denominado 'completeToDo' al prototipo de la clase ToDo
...
function buildToDo(todo, index) {
  // Tu código acá:
let toDoShell = document.createElement('div') //1 <div></div>
toDoShell.setAttribute('class','toDoShell') //2 <div class ='toDoShell'></div>
let toDoText = document.createElement('span') //3 <span></span>
toDoText.innerHTML = todo.description; //4 <span> 'lo que tiene en la propiedad description' </span>
toDoText.id = index// toDoText.setAttribute('id', index); //5 <span id= index> 'lo que tiene en la propiedad description' </span>
// 6, tengo que hacer algo solo si es verdadero entonces hago un if solo y no un if else
if(todo.complete){
  toDoText.setAttribute('class', 'completeText') //6 seteo el atributo en completeText
}
toDoShell.appendChild(toDoText) //7

toDoText.addEventListener('click', completeToDo) // ultimo punto de la home, en las instrucciones de completeToDo  
...
function buildToDos(toDos) { // [{A}, {B}, {C}]
  // Tu código acá:

  let resultado = toDos.map(buildToDo)// la funcion buildToDo ya la tengo creada arriba  y con el metodo de callback map, que por cada elemento del arreglo me va a aplicar lo que yo defini arriba en buildToDo y con esos atributos. 
  // ==> [<div><span>{A}</span></div>, <div><span>{B}</span></div>, div><span>{C}</span></div>]
  return resultado;
}
...
function displayToDos()  { // A todo lo que hicimos [<div><span>{A}</span></div>, <div><span>{B}</span></div>, div><span>{C}</span></div>]
  // Tu código acá:
  let toDoContainer = document.getElementById('toDoContainer'); // 1 <div id="toDoContainer"></div>
  toDoContainer.innerHTML = ''; // 2
  let toDos = buildToDos(toDoItems); 
  toDos.forEach(elemento => { toDoContainer.appendChild(elemento)
  });
...
let newToDo = new ToDo (toDoInput); // {descrp: 'A', complete = false}
...
document.getElementById('addButton').addEventListener('click', addToDo); // cada vez que haga click ejecuta la funcion addToDo
...
//------------------------------------------------mio
let mostrar = ['hola', 'chau', 'estoy'];

newDiv = document.createElement('div'); // creo un <div></div> en mi html, siempre lo crea para abajjo, en orden de entrada
document.querySelector('body').append(newDiv); // selecciono el body y le "appendeo", osea, le agrego el div que cree antes 
newDiv.append(document.createElement('p')); // ahora a mi <div></div> le "appendeo" la creacion de un <p></p> y queda <div><p></p></div>
newDiv.innerText = 'algo mas ' + mostrar // ahora le digo al newDiv que muestre el texto 'hola soy un text'
//------------------------------------------------ extras
chek = document.createElement('label');
document.querySelector('body').append(chek);
chek.append(document.createElement('form'));
